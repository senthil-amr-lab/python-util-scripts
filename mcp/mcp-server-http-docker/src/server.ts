...
app.post("/mcp", async (req: Request, res: Response) => {
  // Create a new transport per request (stateless mode)
  const transport = new StreamableHTTPServerTransport({
    sessionIdGenerator: undefined, // stateless mode (no persistent session store)
  });

  // Optional: cleanup when connection closes
  res.on("close", () => {
    transport.close();
    server.close();
  });
